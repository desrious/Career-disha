...
import { CmsContact } from '../../data/cms';

type ViewType = 'landing' | 'about' | 'insights' | 'contact-us' | 'high-school' | 'plus-two' | 'graduates' | 'working-professional' | 'terms' | 'privacy' | 'refund';
...
export default function LandingFooter({ setView, contact }: LandingFooterProps) {
  return (
    <footer className="bg-slate-900 w-full py-8 sm:py-12 px-4 sm:px-6 md:px-8">
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
        <div className="flex flex-col gap-4">
          ...
          <p className="text-slate-400 text-sm leading-relaxed mb-1">{contact.address}</p>
          ...
          <div className="flex items-center justify-start gap-3 pt-4 flex-wrap">
            <a href={contact.socials.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-10 h-10 bg-slate-800 rounded-full shadow-sm border border-slate-700 flex items-center justify-center text-blue-400 hover:bg-blue-600 hover:text-white transition-all hover:-translate-y-1 hover:shadow-md">
            ...
            <a href={contact.socials.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-10 h-10 bg-slate-800 rounded-full shadow-sm border border-slate-700 flex items-center justify-center text-pink-400 hover:bg-pink-600 hover:text-white transition-all hover:-translate-y-1 hover:shadow-md">
            ...
            <a href={contact.socials.whatsapp} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="w-10 h-10 bg-slate-800 rounded-full shadow-sm border border-slate-700 flex items-center justify-center text-green-400 hover:bg-green-500 hover:text-white transition-all hover:-translate-y-1 hover:shadow-md">
            ...
            <a href={contact.socials.youtube} target="_blank" rel="noopener noreferrer" aria-label="YouTube" className="w-10 h-10 bg-slate-800 rounded-full shadow-sm border border-slate-700 flex items-center justify-center text-red-500 hover:bg-red-600 hover:text-white transition-all hover:-translate-y-1 hover:shadow-md">
        ...
        <div className="flex flex-col gap-4">
          ...
          <ul className="space-y-2">
            ...
            <li><button onClick={() => setView('refund')} className="text-slate-400 text-sm hover:text-primary transition-colors text-left w-full">Refund and Cancellation Policy</button></li>
      ...
      <div className="max-w-7xl mx-auto mt-12 pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-6">
          <Globe className="w-5 h-5 text-slate-500 hover:text-white cursor-pointer transition-colors" onClick={() => window.open(contact.website, '_blank')} />
          <Users className="w-5 h-5 text-slate-500 hover:text-white cursor-pointer transition-colors" onClick={() => window.open(contact.socials.linkedin, '_blank')} />
