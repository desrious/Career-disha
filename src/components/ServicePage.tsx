import { Brain, ArrowRight, CheckCircle2, Map, BookOpen, UserCheck, Star, ArrowLeft, Target } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-3xl mx-auto bg-white/80 p-8 rounded-3xl shadow-xl border border-slate-100 backdrop-blur-lg">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-900 tracking-tight leading-tight mb-6 drop-shadow-sm">
              {data.heroTitle}<span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">{data.heroHighlight}</span>
            </h1>
      ...
      <section className="py-20 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 bg-white/70 p-6 rounded-2xl backdrop-blur-md shadow-sm border border-white max-w-2xl mx-auto">
            <h2 className="text-3xl font-bold text-slate-900 mb-4">{data.problemHeading}</h2>
            <p className="text-lg text-slate-700">{data.problemSubtext}</p>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-12 bg-white/80 p-8 md:p-12 rounded-3xl shadow-xl border border-slate-100 backdrop-blur-lg">
            <div className="lg:w-1/2">
              <h2 className="text-3xl font-bold text-slate-900 mb-6 drop-shadow-sm">{data.offerHeading}</h2>
              <p className="text-lg text-slate-700 mb-8 font-medium">{data.offerSubtext}</p>
              ...
              <div className="bg-gradient-to-br from-blue-100/80 to-sky-100/80 backdrop-blur-md rounded-3xl p-8 shadow-xl border border-white relative z-10">
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-md transform translate-x-4 hover:-translate-y-1 transition-transform border border-slate-100">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h5 className="font-bold text-slate-900">{data.recommendLabel}</h5>
                      <span className="text-sm text-slate-500">Based on assessment</span>
                    </div>
                  </div>
                  <p className="text-lg font-semibold text-primary">{data.recommendValue}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-20 relative z-10 bg-gradient-to-b from-transparent to-slate-900/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            ...
            {[
              { step: '01', title: 'Take Assessment', desc: 'Complete our AI-based psychometric test designed for your profile.', icon: BookOpen },
              { step: '02', title: 'Get Detailed Report', desc: 'Receive a comprehensive analysis of your interests, personality, and aptitudes.', icon: Target },
              { step: '03', title: 'Expert Guidance', desc: '1-on-1 counseling session to decode the report and finalize your path.', icon: UserCheck },
            ].map((item, index) => {
